...
#实时日志查询格式bash query_ly_ocslog.sh 号码
#归档日志查询格式bash query_ly_ocslog.sh 号码 2024-01-04-16
...
if [ -z "$timestamp" ]; then
    ...
    ansible -i ~/hosts/hosts_wh ocs_ly -m shell -a "zgrep -h '$number' /home/ucp/outCallService/tomcat*/logs/outCallService.log.$timestamp*.gz"
fi

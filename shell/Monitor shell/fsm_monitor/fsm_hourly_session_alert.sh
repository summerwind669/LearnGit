...
QUERY="SELECT host_ip, AVG(current_session) AS avg_current_session FROM fsm_session_data_5min WHERE DATE(insert_time) = '$CURRENT_DATE' AND HOUR(insert_time) = '$CURRENT_HOUR' GROUP BY host_ip"
...
while read -r line; do
    ...
    avg_current_session=$(echo "$line" | awk '{ print $2 }')
    threshold=$(echo "scale=2; $TOTAL_AVG * 0.9" | bc)
    
    if (( $(echo "$avg_current_session < $threshold" | bc -l) )); then
        echo "Warning: Host IP $host_ip has average current session ($avg_current_session) lower than 90% of total average ($TOTAL_AVG) for hour $CURRENT_HOUR"

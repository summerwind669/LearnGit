...
if [ $now_timestamp -ge $alarm_starttime -a $now_timestamp -le $alarm_endtime ]; then
    ...
    while read -r line; do
        ...
        if (( $(echo "$avg_current_session < $threshold" | bc -l) )); then
            echo "通信云$province$data_center, Host IP: $host_ip - 从 $CURRENT_HALF_HOUR 到 $CURRENT_HOUR 期间并发量平均值 ($avg_current_session) 小于全部节点平均值($TOTAL_AVG) 的70%，当前值(${current_minimum_node_session_percent}%)，请检查服务是否正常。|1|int"
            # 这里可以添加发送告警的逻辑，比如通过邮件、短信接口等方式
        else
            echo "通信云$province$data_center, Host IP: $host_ip - 从 $CURRENT_HALF_HOUR 到 $CURRENT_HOUR 期间并发量平均值 ($avg_current_session) 达到或超过全部节点平均值($TOTAL_AVG) 的70%，当前值(${current_minimum_node_session_percent}%)，所有节点运行正常。|0|int"
        fi
    done <<< "$RESULT"
else

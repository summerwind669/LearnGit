...
# 过滤出时间范围内的日志，过滤出时间范围内的日志，并统计关键字的出现次数
...
if [[ $ip_address == 192.* ]]; then
    echo "通信云洛阳外呼LB一分钟内上行流量为：|$resp_evt_count|int"
    echo "通信云洛阳外呼LB一分钟内下行流量为：|$cmd_count|int"
elif [[ $ip_address == 172.* ]]; then
    echo "通信云淮安外呼LB一分钟内上行流量为：|$resp_evt_count|int"
    echo "通信云淮安外呼LB一分钟内下行流量为：|$cmd_count|int"
else
